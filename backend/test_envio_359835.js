 ...
 * ─────────────────────────────────────────────────────────────────
 *  Envía el correo completo de la atención 359835 (MARIELA HERNANDEZ, CC 22097046)
 ...
 *
 *  Ejecución:  node test_envio_359835.js
...
const NUMERO_DOCUMENTO      = '22097046';  // CC de MARIELA HERNANDEZ
const TIPO_DOCUMENTO        = 'CC';
...
async function main() {
  ...
  console.log(`    Paciente esperado: MARIELA HERNANDEZ — CC ${NUMERO_DOCUMENTO}`);
  console.log('🏥 ══════════════════════════════════════════════════════════════════\n');

  // ── Paso 1: Obtener datos del paciente ──────────────────────────────────
  console.log('🔍 Buscando paciente en el portal...');
  let paciente;
  try {
  ...
  if (!paciente) {
    console.warn('⚠️  No se encontró el paciente por atención en la vista del portal.');
    console.warn('   Usando documento configurado manualmente en el script.');
    paciente = {
      NOMBRE_COMPLETO: 'MARIELA HERNANDEZ',
      numero_documento: NUMERO_DOCUMENTO,
      email: CORREO_DESTINO_PRUEBA,
  ...
  console.log(`   Correo destino : ${correoFinal || '⚠️  SIN CORREO DESTINO'}`);
  ...
  // Mostrar qué órdenes trae el payload
  const totalOrdenes = (payload.clinico?.ordenes || []).length;
  const totalFormulacion = (payload.clinico?.formulacion?.length) || 0;
  console.log(`   Órdenes en payload    : ${totalOrdenes}`);
  console.log(`   Items de formulación  : ${totalFormulacion}`);
  ...
  try {
    ...
    // 2️⃣ + 3️⃣  Clasificar TODAS las órdenes → un PDF por tipo
    ...
    console.log(`     Laboratorio    : ${(clasificados.laboratorio   || []).length} grupo(s)`);
    console.log(`     Imagenología   : ${(clasificados.imagenologia  || []).length} grupo(s)`);
    console.log(`     Medicamentos   : ${(clasificados.medicamentos  || []).length} grupo(s)`);
    console.log(`     Incapacidades  : ${(clasificados.incapacidades || []).length} grupo(s)`);
    console.log(`     Otras          : ${(clasificados.otras         || []).length} grupo(s)\n`);
    ...
    if (rsMedicamentos.length) {
      ...
      console.log('   ⏭️  Sin fórmula médica — se omite');
    }
  ...
  console.log(`   PDFs generados    : ${adjuntos.length}`);
  adjuntos.forEach((a, i) => console.log(`     ${i + 1}. ${a.filename}`));
